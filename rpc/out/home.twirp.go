...
/*
Package home is a generated twirp stub package.
...
// ===========================
// CrypticTown Protobuf Client
// ===========================

type crypticTownProtobufClient struct {
	urlBase string
	client  *http.Client
}

// NewCrypticTownProtobufClient creates a Protobuf client that implements the CrypticTown interface.
// It communicates using protobuf messages and can be configured with a custom http.Client.
func NewCrypticTownProtobufClient(addr string, client *http.Client) CrypticTown {
	return &crypticTownProtobufClient{
...
// =======================
// CrypticTown JSON Client
// =======================

type crypticTownJSONClient struct {
	urlBase string
	client  *http.Client
}

// NewCrypticTownJSONClient creates a JSON client that implements the CrypticTown interface.
// It communicates using JSON requests and responses instead of protobuf messages.
func NewCrypticTownJSONClient(addr string, client *http.Client) CrypticTown {
	return &crypticTownJSONClient{
		urlBase: urlBase(addr),
		client:  withoutRedirects(client),
	}
}
...
// ==========================
// CrypticTown Server Handler
// ==========================

type crypticTownServer struct {
	CrypticTown
	hooks *twirp.ServerHooks
}

func NewCrypticTownServer(svc CrypticTown, hooks *twirp.ServerHooks) TwirpServer {
	return &crypticTownServer{
		CrypticTown: svc,
		hooks:       hooks,
	}
}
...
func (s *crypticTownServer) writeError(ctx context.Context, resp http.ResponseWriter, err error) {
	writeError(ctx, resp, err, s.hooks)
}

// CrypticTownPathPrefix is used for all URL paths on a twirp CrypticTown server.
// Requests are always: POST CrypticTownPathPrefix/method
// It can be used in an HTTP mux to route twirp requests along with non-twirp requests on other routes.
const CrypticTownPathPrefix = "/twirp/CrypticTown/"

func (s *crypticTownServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	ctx := req.Context()
	ctx = ctxsetters.WithPackageName(ctx, "")
	ctx = ctxsetters.WithServiceName(ctx, "CrypticTown")
...
func (s *crypticTownServer) ServiceDescriptor() ([]byte, int) {
	return twirpFileDescriptor0, 0
}

func (s *crypticTownServer) ProtocGenTwirpVersion() string {
	return "v5.0.0"
}
